 ...
 * DebitNotePackage
 *
 * @ORM\Table(name="debit_note_package")
 * @ORM\Entity(repositoryClass="GCRM\CRMBundle\Repository\PackageToProcessRepository")
 * @ORM\HasLifecycleCallbacks
 */
class DebitNotePackage
{
    /**
     * @ORM\OneToMany(targetEntity="Wecoders\EnergyBundle\Entity\DebitNotePackageRecord", mappedBy="package", cascade={"remove"})
     */
    ...
    public function getCountWaitingToProcess()
    {
        ...
        foreach ($this->packageRecords as $packageRecord) {
            if ($packageRecord->getStatus() == DebitNotePackageRecordModel::STATUS_WAITING_TO_PROCESS) {
                $count++;
            }
        }
    ...
    public function getCountWaitingToGenerate()
    {
        ...
        /** @var DebitNotePackageRecord $packageRecord */
        foreach ($this->packageRecords as $packageRecord) {
            if ($packageRecord->getStatus() == DebitNotePackageRecordModel::STATUS_WAITING_TO_GENERATE) {
                $count++;
            }
        }
